  ...
  for (const t of tickets) {
    ...
    try {
      ...
      if (won) {
        ...
        await supabase.from('tickets').update({ claimed: true }).eq('id', t.id)
        // Referral credit: if this loser was referred, credit their referrer 2%
        // of the loss. WRAPPED - a referral failure must never break settlement.
        // Idempotent: unique(ticket_id) in the RPC blocks double-credit on re-run.
        try {
          await supabase.rpc('credit_referrer_for_loss', {
            p_wallet: t.wallet_address, p_ticket_id: t.id,
            p_loss_usd: Number(t.stake), p_sol_price: solPrice,
          })
        } catch (e) { console.error('referral credit failed (single)', t.id, e?.message) }
      }
    } catch (err) {
  ...
  for (const [comboId, ls] of Object.entries(groups)) {
    try {
      ...
      if (allWon) {
        ...
        await mark()
        // Referral credit on the combo loss (once per combo, keyed on ls[0].id).
        // Wrapped; idempotent via unique(ticket_id).
        try {
          await supabase.rpc('credit_referrer_for_loss', {
            p_wallet: wallet, p_ticket_id: ls[0].id,
            p_loss_usd: stake, p_sol_price: solPrice,
          })
        } catch (e) { console.error('referral credit failed (combo)', comboId, e?.message) }
      }
    } catch (err) {
