...
export default function BattleDetailPage() {
    ...
    try {
      ...
      try {
        const code = await createBetShare({
          legs: [{ battle_id: battle.id, side: selectedSide! }],
      ...
      // Slip reset + refetches deferred to the share modal's onClose -
      // running them here unmounted the just-opened modal after ~2s.
    } catch (e: any) {
  ...
  async function handlePlaceBetArc() {
    ...
    try {
      ...
      // battle.id is a UUID, so the old `arc_battle_id || battle.id` fallback
      // would have thrown inside BigInt(). An unmirrored battle simply cannot
      // be bet on Arc - /api/arc-quote refuses to quote one.
      if (battle.arc_battle_id == null) {
        showToast('This battle is not on Arc yet', 'error')
        return
      }

      // No client-side odds any more. /api/arc-quote prices the leg server-side
      // and signs it; the contract verifies that signature. The bettor cannot
      // choose their own payout - which is exactly how the old contract was
      // drained.
