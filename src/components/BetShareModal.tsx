...
export default function BetShareModal({
  open, onClose, code, coinA, coinB, side, odds, stake, legCount,
}: BetShareModalProps) {
          ...
          >
            ...
            >
              ...
              <p className="text-[13px]" style={{ color: 'rgba(255,255,255,0.6)' }}>
                {coinA} vs {coinB}{isCombo ? ` +${extraLegs} more` : ''}
              </p>
              <p className="mt-1 text-[15px] font-semibold text-white">
                {pickLabel} <span style={{ color: COLORS.accent }}>· {odds.toFixed(2)}x{isCombo ? ' combo' : ''}</span> <span style={{ color: 'rgba(255,255,255,0.6)' }}>· ${stake}</span>
              </p>
            </div>
